"""
MeetScribe - Local Meeting Recorder (macOS Edition)
Flask backend for audio recording and transcription using mlx-whisper (Apple Silicon).

macOS audio setup:
  - Create a Multi-Output Device in Audio MIDI Setup (speakers + BlackHole 2ch)
  - Create an Aggregate Device (BlackHole 2ch + your mic)
  - Set your meeting app output to the Multi-Output Device
  - In MeetScribe, select the Aggregate Device as your mic input
  - No loopback device needed — the Aggregate Device already combines both streams
"""
...
# ---------------------------------------------------------------------------
# Whisper model (loaded once, reused) — mlx-whisper for Apple Silicon
# ---------------------------------------------------------------------------
...
def _warm_mlx_model():
    global _whisper_model_loaded
    with _whisper_lock:
        if not _whisper_model_loaded:
            import mlx_whisper
            print(f"[Whisper] Warming mlx-whisper model ({MLX_MODEL})...")
            mlx_whisper.transcribe(np.zeros(16000, dtype=np.float32), path_or_hf_repo=MLX_MODEL, verbose=False)
...
def list_input_devices():
    ...
    On macOS, BlackHole and Aggregate Devices show up as regular input devices.
    """
    devices = []
    for i, dev in enumerate(sd.query_devices()):
        if dev['max_input_channels'] > 0:
            ...
            is_loopback = (
                ...
                'meetscribe input' in name_lower or
...
def start_recording_meeting(meeting_name: str, mic_device=None, loopback_device=None):
    ...
    def record_loop():
        ...
        try:
            ...
            if not streams:
                print("[Record] No valid devices — using default input")
...
def _do_transcribe(audio_path: str, meeting_id: str, meeting_name: str):
    ...
    import mlx_whisper
    print(f"[Transcribe] Running mlx-whisper on {audio_path}...")
    ...
    for seg in segments:
        ...
        if not text or text == prev_text:
            continue
        start_hms = seconds_to_hms(seg.get("start", 0))
...
# ---------------------------------------------------------------------------
# Routes — identical to Windows version
# ---------------------------------------------------------------------------
...
if __name__ == '__main__':
    print("Starting MeetScribe (macOS)...")
    print(f"Recordings dir: {app.config['RECORDINGS_DIR']}")
    threading.Thread(target=_warm_mlx_model, daemon=True).start()
